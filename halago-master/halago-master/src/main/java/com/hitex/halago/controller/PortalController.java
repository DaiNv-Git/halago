        ...
        try {
            ...
            if (baseRequest.getWsRequest() != null) {
                if (!Constant.BRAND.equals(role) || !Constant.INFLUENCER.equals(role)) {
                    int id = jsonNode.get("id").asInt();
                    String address = jsonNode.get("address").asText();
                    String address_en = jsonNode.get("address_en").asText();
                    String email = jsonNode.get("email").asText();
                    String hotline = jsonNode.get("hotline").asText();
                    String fb = jsonNode.get("facebook").asText();
                    String zalo = jsonNode.get("zalo").asText();
                    String company = jsonNode.get("company").asText();
                    String youtube = jsonNode.get("youtube").asText();
                    String tiktok = jsonNode.get("tiktok").asText();
                    String instagram = jsonNode.get("instagram").asText();
                    if (!StringUtils.isEmpty(address) && !StringUtils.isEmpty(email) && !StringUtils.isEmpty(hotline)) {
                        Footer footer = footerRepository.findFooterById(id);
